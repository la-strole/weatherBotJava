...
public class MessageHandler {
     ...
     * It retrieves the list of coordinates for the city (or cities if there are multiple
     * with the same name) using the GeocodingApiOpenWeather class.
     * If there is only one city found, it sends the current weather information for that city.
     * If there are multiple cities found, it sends a message to the user to choose a city.
     */
    public void handleMessage() {
        ...
        if (geocodingApiResponse.length() == 1) {
            ...
            MultipleCitiesChoise.sendMessageToChooseCity(geocodingApiResponse, telegramClient, chatId, msgId, language);
        }
    }
}
